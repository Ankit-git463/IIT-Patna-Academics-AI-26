...
#define MAX_ITERATIONS 20  // Limit the number of iterations
...
// Function to parse topology from user input
void parseTopologyInput() {
    ...
    // Read router names
    for (int i = 0; i < numRouters; i++) {
        printf("Enter name for router %d: ", i + 1);
        scanf(" %c", &routers[i].name);
    ...
    for (int i = 0; i < numLinks; i++) {
        ...
        int cost;
        printf("Enter link %d (router1 router2 cost): ", i + 1);
        scanf(" %c %c %d", &router1, &router2, &cost);

        int id1 = -1, id2 = -1;
        for (int j = 0; j < numRouters; j++) {
            if (routers[j].name == router1) id1 = j;
            if (routers[j].name == router2) id2 = j;
        }

        if (id1 == -1 || id2 == -1) {
            fprintf(stderr, "Invalid router names: %c or %c.\n", router1, router2);
            exit(EXIT_FAILURE);
        }
...
int main() {
    // Parse the topology from user input
    parseTopologyInput();
    ...
    for (int i = 0; i < numRouters; i++) {
        ...
        if (routerThreads[i] == NULL) {
            fprintf(stderr, "Error creating thread for router %c\n", routers[i].name);
            exit(EXIT_FAILURE);
        }
    }
