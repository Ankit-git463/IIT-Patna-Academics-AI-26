...
void parseTopologyFile(const char *filename) {
    ...
    // // Check for "END" line explicitly
    // char endCheck[4];
    // if (fscanf(file, "%s", endCheck) == 1 && strcmp(endCheck, "END") == 0) {
    //     printf("Topology file parsing complete.\n");
    // } else {
    //     fprintf(stderr, "Invalid topology file format. Expected 'END' at the end.\n");
    //     fclose(file);
    //     exit(EXIT_FAILURE);
    // }
