...
DWORD WINAPI routerThreadFunction(LPVOID arg) {
    ...
    while (iterationCount < MAX_ITERATIONS) {
        ...
        // Print routing table
        WaitForSingleObject(router->mutex, INFINITE);
        printf("Iteration %d Router %c Routing Table:\n", iterationCount, router->name);
...
void parseTopologyFile(const char *filename) {
    ...
    if (numRouters > MAX_ROUTERS) {
        fprintf(stderr, "Number of routers exceeds the maximum allowed.\n");
        exit(EXIT_FAILURE);
    }
    ...
    while (fscanf(file, " %c %c %d", &router1, &router2, &cost) == 3) {
        ...
        if (id1 == -1 || id2 == -1) {
            fprintf(stderr, "Invalid router names in topology file.\n");
            fclose(file);
            exit(EXIT_FAILURE);
        }
