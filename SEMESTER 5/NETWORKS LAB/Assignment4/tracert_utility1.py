...
def tracert(dest_ip, max_ttl, packet_size, timeout, pings_per_hop, delay_between_pings, source_ip=None, output_file=None):
    results = []
    
    print(f"Tracing route to {dest_ip} with max TTL {max_ttl}, packet size {packet_size} bytes, timeout {timeout}s, {pings_per_hop} ping(s) per hop")
    
    for ttl in range(1, max_ttl + 1):
        for ping in range(pings_per_hop):
            ...
            if reply is None:
                ...
                results.append(f"{ttl}\t{reply.src}\t{round(rtt, 2)} ms (TTL expired)")
            elif reply.type == 0:  # Echo reply (destination reached)
                print(f"{ttl}\t{reply.src}\t{round(rtt, 2)} ms (Destination reached)")
                results.append(f"{ttl}\t{reply.src}\t{round(rtt, 2)} ms (Destination reached)")
                break
            else:
                print(f"{ttl}\t{reply.src}\t{round(rtt, 2)} ms (Unknown response)")
                results.append(f"{ttl}\t{reply.src}\t{round(rtt, 2)} ms (Unknown response)")
...
def validate_inputs(dest_ip, max_ttl, packet_size, timeout, pings_per_hop, delay_between_pings, source_ip):
    try:
        # Validate destination IP
        socket.inet_aton(dest_ip)
    except socket.error:
        raise ValueError(f"Invalid destination IP: {dest_ip}")
...
if __name__ == "__main__":
    try:
        ...
        # Validate inputs
        validate_inputs(dest_ip, max_ttl, packet_size, timeout, pings_per_hop, delay_between_pings, source_ip)
        
        # Call the function
        tracert(dest_ip, max_ttl, packet_size, timeout, pings_per_hop, delay_between_pings, source_ip, output_file)
