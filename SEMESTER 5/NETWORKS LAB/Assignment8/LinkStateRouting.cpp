...
void dijkstra(string routerName) {
    ...
    while (visited.size() < router.allRouters.size()) {
        ...
        // Display the routing table for this router in a tabular format with the iteration number
        lock_guard<mutex> lock(displayMutex);
        cout << "Routing Table for Router " << routerName << " - Iteration " << iteration++ << ":\n";
        cout << "+-------------+----------+\n";
...
void routerThreadFunction(string routerName) {
    ...
    cout << "Terminating router " << routerName << ".\n";
}
