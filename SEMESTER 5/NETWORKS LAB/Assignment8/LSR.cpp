...
void dijkstra(string routerName) {
    ...
    // Display the routing table for this router
    {
        lock_guard<mutex> lock(displayMutex);
        cout << "Router " << routerName << " Routing Table (Converged):" << endl;
        for (const auto& [dest, cost] : distances) {
            cout << "Destination: " << dest << ", Cost: " << (cost == numeric_limits<int>::max() ? "INF" : to_string(cost)) << endl;
        }
        cout << "---------------------------" << endl;
    }
}
...
void routerThreadFunction(string routerName) {
    ...
    cout << "Network has converged. Terminating router " << routerName << "." << endl;
}
