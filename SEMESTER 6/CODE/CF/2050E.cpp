...
int helper(int i , int x ,int y  ){
    ...
    if ( i == n ) return 0 ; 
    // dp[i] = number of min differces from 0 to i . 
    // taken from first 
    ...
    if (x<a.size()){
        if (c[i] == a[x]){
            first = helper(i+1 , x+1 , y );
        }
        else{
            first = helper(i+1 , x+1 , y ) +1 ; 
        }
    } 
    ...
    if(y< b.size() ){
        if (c[i] == b[y]){
            second = helper(i+1 , x , y+1 );
        }
        else{
            second = helper(i+1 , x , y+1 ) +1 ; 
        }
    }

    return dp[x][y] = min(first,second);
