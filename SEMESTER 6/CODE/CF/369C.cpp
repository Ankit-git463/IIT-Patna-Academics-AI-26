...
bool dfs(int start, vector< vector< pair <int,bool> >  > &graph , int par , set<int>& st, bool parp){
	
    // make subtree problem to be false first 
    bool subtree=false;
	...
	for(auto it : graph[start]){
        ...
        // newnode is same as parent then continue
		if(newnode==par) continue;


        // current problem vertex 
		bool p = it.second;

        // internal problem 
		bool internalp = dfs(newnode , graph , start , st , p);
        

		if( p || internalp ) subtree=true;
	}


	if( parp && !subtree ) st.insert(start);
